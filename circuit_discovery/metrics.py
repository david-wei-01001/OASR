...
# --------------------------------------------------------------------------------------
# N-way classification (Articulatory Index: consonant/vowel/etc.)
# --------------------------------------------------------------------------------------
# Parallel to the DiscoGP-specific task losses and evaluate_good_bad_accuracy
# above, but for a pooled N-way classification head (CircuitHubertClassifier)
# rather than per-position LM logits. `logits` here is already [batch,
# num_classes] -- the model call itself (via the classifier wrapper) has
# already done backbone forward + pooling + the linear head, so these
# functions don't need to know anything about seq_lens/pooling/hidden size.

def discogp_classification_fidelity_loss(batch: dict[str, Any], logits: torch.Tensor) -> torch.Tensor:
    """
    default cross-entropy loss against the true label.
...
def discogp_classification_completeness_loss(
    batch: dict[str, Any],
    logits: torch.Tensor,
) -> torch.Tensor:
    """
    default complement loss: pushes the complement circuit's predictions
    toward the uniform distribution over classes, so it becomes uninformative
    about the label rather than (say) actively predicting the correct label
    or actively predicting some other specific wrong label.
    """
    num_classes = logits.shape[-1]
    uniform = logits.new_full((logits.shape[0], num_classes), 1.0 / num_classes)
    return F.cross_entropy(logits, uniform)
...
def evaluate_classification_accuracy(
    *,
    ...
    """
    evaluate top-1 accuracy of a pooled N-way classifier (CircuitHubertClassifier)
    over its finalized/candidate circuit.
